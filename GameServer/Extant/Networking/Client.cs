                ...
                case (ClientState.Varifying):
                    {
                        ...
                        {
                            if (p.Type == (Int32)GameServerPackets.PacketType.Verify_Details_g)
                            {
                                verifyBuild = (p as GameServerPackets.Verify_Details_g).build;
                                verifyUsername = (p as GameServerPackets.Verify_Details_g).username;
                                verifyPassword = (p as GameServerPackets.Verify_Details_g).password;

                                //Check if gameversion is correct
                                if (verifyBuild == GameVersion.Build)
                                {
                                    state = ClientState.Connected;
                                    connection.SendPacket(new GameServerPackets.Verify_Result_c(GameServerPackets.Verify_Result_c.VerifyReturnCode.Success));
                                }
                                else
                                {
                                    connection.SendPacket(new GameServerPackets.Verify_Result_c(GameServerPackets.Verify_Result_c.VerifyReturnCode.IncorrectVersion));
                                    this.Stop();
                                }
                            }
                            else
