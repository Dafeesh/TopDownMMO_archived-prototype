    ...
    public class GameHandler : ThreadRun
    {
        ...
        private void SortVarifiedClients()
        {
            ...
            while ((varifiedClient = hostServer.GetVarifiedClient()) != null)
            {
                    ...
                    if (g != null)
                    {
                        foreach (Player p in g.Players)
                        {
                            if (p.Username == varifiedClient.VerifyUsername)
                            {
                                ...
                                {
                                    varifiedClient.SendPacket(new GameServerPackets.Verify_Result_c(GameServerPackets.Verify_Result_c.VerifyReturnCode.IncorrectPassword));
                                    varifiedClient.Stop();
                ...
                if (!found)
                {
                    varifiedClient.SendPacket(new GameServerPackets.Verify_Result_c(GameServerPackets.Verify_Result_c.VerifyReturnCode.DoesNotExist));
                    varifiedClient.Stop();
