...
namespace Extant.Networking
{
    ...
    public abstract class Packet
    {
        ...
        /// <summary>
        /// Returns a UInt32 that is read and removed from beginning of List of Bytes.
        /// </summary>
        /// <param name="buff">The array for data to be taken from.</param>
        public static UInt32 TakeUInt32(ref List<Byte> buff)
        {
            if (buff.Count < sizeof(UInt32))
                throw new ArgumentOutOfRangeException("Buffer not big enough to pull UInt32.");

            UInt32 read = BitConverter.ToUInt32(buff.ToArray(), 0);
            buff = buff.Skip(sizeof(UInt32)).ToList();
            return read;
        }
        ...
        /// <summary>
        /// Returns the Bytes that make up an UInt32.
        /// </summary>
        public static Byte[] GetBytes_UInt32(UInt32 i)
        {
            return BitConverter.GetBytes(i);
        }
